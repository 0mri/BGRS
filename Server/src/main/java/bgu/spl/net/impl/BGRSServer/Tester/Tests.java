...
public class Tests implements Runnable {
            ...
            for (int i = 0; i < numThreads; i++) {
                ...
                new Thread(() -> {
                    try {
                        ...
                        outputs.add(
                                commandsProcessors.get(tempI).sendCommand("STUDENTREG " + username + " " + username));
            ...
            for (int i = 0; i < numThreads; i++) {
                ...
                new Thread(() -> {
                    try {
                        ...
                        outputs.add(commandsProcessors.get(tempI).sendCommand("LOGIN " + username + " " + username));
                        outputs.add(commandsProcessors.get(tempI).sendCommand("COURSESTAT " + tempP.getCourseNum())); // Cant
                                                                                                                      // access
                                                                                                                      // admin
                                                                                                                      // commands
                        outputs.add(commandsProcessors.get(tempI).sendCommand("STUDENTREG L" + tempI + " L" + tempI)); // Shouldn't
                                                                                                                       // work
                                                                                                                       // after
                                                                                                                       // you
                                                                                                                       // are
                                                                                                                       ...
                                                                                                                       // logged
                                                                                                                       // in
                        outputs.add(commandsProcessors.get(tempI).sendCommand("ADMINREG L" + tempI + " L" + tempI)); // Shouldn't
                                                                                                                     // work
                                                                                                                     // after
                                                                                                                     // you
                                                                                                                     // are
                                                                                                                     ...
                                                                                                                     // logged
                                                                                                                     // in
                        outputs.add(commandsProcessors.get(tempI).sendCommand("STUDENTSTAT RON" + (tempI - 1))); // ADMIN
                                                                                                                 // COMMAND
                        outputs.add(commandsProcessors.get(tempI).sendCommand("LOGOUT"));
                    } catch (Exception e) {
                        e.printStackTrace();
    ...
    String tryBulkAdminLogicalTests() {
        ...
        try {
            ...
            for (int i = 0; i < numThreads; i++) {
                ...
                new Thread(() -> {
                    try {
                        ...
                        outputs.add(commandsProcessors.get(tempI).sendCommand("LOGIN " + username + " " + username));
                        outputs.add(commandsProcessors.get(tempI).sendCommand("COURSEREG " + p.getCourseNum())); // Shouldn't
                                                                                                                 // work
                                                                                                                 // since
                                                                                                                 // admin
                        outputs.add(commandsProcessors.get(tempI).sendCommand("KDAMCHECK " + p.getCourseNum())); // Shouldn't
                                                                                                                 // work
                                                                                                                 // since
                                                                                                                 // admin
                        outputs.add(commandsProcessors.get(tempI).sendCommand("MYCOURSES")); // Shouldn't work since
                                                                                             // admin
                        outputs.add(commandsProcessors.get(tempI).sendCommand("UNREGISTER " + p.getCourseNum())); // Shouldn't
                                                                                                                  // work
                                                                                                                  // since
                                                                                                                  // admin
                        outputs.add(commandsProcessors.get(tempI).sendCommand("ISREGISTERED " + p.getCourseNum()));// Shouldn't
                                                                                                                   // work
                                                                                                                   // since
                                                                                                                   // admin
                        outputs.add(commandsProcessors.get(tempI).sendCommand("LOGOUT"));
            ...
            int numTimesSuccess = 0;
            for (String s : outputs) {
                if (s.equals("ACK 2") || s.equals("ACK 6") || s.equals("ACK 5") || s.equals("ACK 10")
                        || s.equals("ACK 9")) { // STUDENT REG SHALL FAIL SINCE WE ALREADY LOGGED
    ...
    public String testRegisterCourseWithKdam() {
        ...
        try {
            ...
            for (int i = 0; i < actualSpots + 2; i++) {
                ...
                new Thread(() -> {
                    try {
                        ...
                        outputs.add(
                                commandsProcessors.get(finalI).sendCommand("ISREGISTERED " + finalP.getCourseNum()));
                        outputs.add(commandsProcessors.get(finalI)
                                .sendCommand("COURSEREG " + finalP.getKdamCourses().get(0).getCourseNum()));
                        outputs.add(commandsProcessors.get(finalI).sendCommand("COURSEREG " + finalP.getCourseNum()));
                        outputs.add(
                                commandsProcessors.get(finalI).sendCommand("ISREGISTERED " + finalP.getCourseNum()));
                        outputs.add(commandsProcessors.get(finalI)
                                .sendCommand("UNREGISTER " + finalP.getKdamCourses().get(0).getCourseNum()));
            ...
            if (numTimesSuccess != (int) (actualSpots * 4 + 4 + numberOfExtraSpacesInKdam)
                    || numTimesRegistered != actualSpots || numTimesUnsuccessful != (8 - numberOfExtraSpacesInKdam * 2)
                    || numTimesNotRegistered != actualSpots + 4
                    || numTimeSucUnregister != (int) (actualSpots * 2 + numberOfExtraSpacesInKdam))
                response = "CourseReg Test - Kdam Test 2 Failed (Registering Kdam Courses And Then Course)";
            else
                response = "CourseReg Test - Kdam Test 2 Passed";
        } catch (Exception e) {
            e.printStackTrace();
        }
        return response;
